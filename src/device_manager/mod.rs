//! Device ownership management with integrated scheduling.
//!
//! This crate owns the `DeviceManager` which tracks device ownership and wraps
//! the `Scheduler` for scheduling operations. This avoids circular dependencies
//! between `common` and `scheduler` while providing a unified API for `backdoor`
//! and `cluster` crates.
